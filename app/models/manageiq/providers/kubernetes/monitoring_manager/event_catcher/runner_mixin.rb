module ManageIQ::Providers::Kubernetes::MonitoringManager::EventCatcher::RunnerMixin
  ...
  def extract_event_data(event)
    ...
    event[:ems_ref] = incident_identifier(labels, annotations)
    ...
    timestamp = event[:resolved] ? event["endsAt"] : event["startsAt"]
    {
      ...
      :ems_ref    => incident_identifier(labels, annotations),
  ...
  def find_target(annotations, labels)
    ret = nil
    if annotations['miqTarget'] == 'ContainerNode'
      ...
      if node
        ...
        $cn_monitoring_log.error("Could not find node from labels: [#{labels}] defaulting to ems") unless node
      end
    end
    ret ||  {
  ...
  def incident_identifier(labels, annotations)
